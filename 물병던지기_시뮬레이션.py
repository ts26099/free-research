...
A partially filled bottle is released spinning and followed during free
flight.  The water is discretised into elements that move under the
fictitious forces of the rotating frame, either as
...
As the water redistributes, the moment of inertia grows and, angular
momentum being conserved in free flight, the rotation slows down: that is
what lets the bottle land upright.

This file is the single-file version of the repository, meant to be pasted
into one Google Colab cell.  It is generated by ``build_colab.py`` from
``constants.py``, ``parameters.py``, ``neighbours.py``, ``kinetics.py``,
``parcels.py``, ``beads.py``, ``simulate_flip.py`` and ``gradio_app.py``;
edit those and re-run the builder rather than editing this file.
...
Quick start — the physical quantities can be set three ways::
    ...
    params = Parameters(bottle_height=0.31, bottle_radius=0.035,
                        water_mass=0.4, omega_0=25, model="parcels")
    results = run(params); report(results); draw(results)

    results = simulate(water_mass=0.4)    # or just override what you need
...
From a shell it also works as a script::
    ...
    python 물병던지기_시뮤레이션.py --bottle-height 0.31 --water-mass 0.4
...
class Parameters:
    ...
    omega_0: float = _field(
        20.0, "float", "rad/s", "초기 각속도 / initial angular velocity",
...
def step_matrix(omega, omega_dot, tau, y_cm, z_cm, dt, gravity=0.0, angle=0.0,
    ...
    # 자유비행에서 중력은 0 이다. 부호 약속은 원판 모델과 같다 — 축 방향
    # 성분이 gravity * cos(angle) 이다.
...
PRESETS = [
    ...
    ("1.4 L 기본 / default", 0.28, 0.040, 0.028, 0.35, 20.0, "slices", 19),
    ("0.5 L 생수병 / small bottle", 0.22, 0.032, 0.020, 0.18, 22.0, "slices",
     19),
    ("단면 분해 / cross section resolved", 0.28, 0.040, 0.028, 0.35, 20.0,
     "parcels", 19),
    ("구슬 / beads (forces)", 0.28, 0.040, 0.028, 0.35, 20.0, "beads", 19),
    ("달에서 / on the Moon", 0.28, 0.040, 0.028, 0.35, 20.0, "slices", 19),
]
...
def filling_scan(*values, fractions=(0.05, 0.15, 0.25, 0.35, 0.45)):
